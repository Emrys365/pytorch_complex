...
@pytest.mark.parametrize(
    "shape",
    [(1, 2), (1, 4), (1, 8), (2, 4), (10, 5), (100, 10), (1000, 20)],
)
def test_symeig(shape):
    Z = ComplexTensor(torch.rand(*shape, 1), torch.rand(*shape, 1))
    ...
    numpy.testing.assert_allclose(
        numpy.matmul(Z.numpy(), eigvecs_torch.numpy()),
        eigvecs_torch.numpy() * eigvals_torch.unsqueeze(-2).numpy(),
        atol=1e-5,
    )
